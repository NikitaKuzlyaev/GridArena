...
class SubmissionService(ISubmissionService):
    ...
    async def check_submission(
        ...
        selected_problem: SelectedProblem = (
            await self.context_service.context.get_selected_problem(selected_problem_id=selected_problem_id)
        )
        problem_card: ProblemCard = (
            await self.context_service.context.get_problem_card_from_selected_problem()
        )
        problem: Problem = (
            await self.context_service.context.get_problem_from_problem_card(problem_card=problem_card)
        )
    ...
    async def get_possible_reward(
        ...
        quiz_field: QuizField = (
            await self.context_service.context.get_quiz_field_from_problem_card(problem_card=problem_card)
        )
        contest: Contest = (
            await self.context_service.context.get_contest_from_quiz_field(quiz_field=quiz_field)
        )
